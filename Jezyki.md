...
void minMax(double tab[],int n, double *pmin, double *pMax){
	*pMax=liczby[0];
	*pmin=liczby[0];
	int i;
	for (i=1;i<n;i++){
...
void zapisz(struct student Studenci[], int iloscStudentow){
	int k;
	FILE *plik;
	const char * const nazwaPliku="studenci.txt";
...
int dodaj(struct student Studenci[], int iloscStudentow){
	char odp, bufor[32];
	int oc;
	printf("Czy chcesz dodac kolejnych studentow? t/n \n");
	odp=getchar();
	getchar();
	do{
...
void wczytaj(struct student Studenci[]){
	int i=0;
	FILE *plik;
	const char *const nazwaPliku="studenci.txt";
